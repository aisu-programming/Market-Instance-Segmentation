...
class Trainer():
    ...
    def demo(self):
        ...
        for i in range(3):
            for j in range(5):
                axs[i][j].axis('off')
                axs[i][j].imshow(img[i*5+j][..., ::-1])
                # axs[i][j].set_title(f"Truth: {int(amount_real[i*5+j]*100)}% | " + \
                #                     f"Prediction: {int(amount_pred[i*5+j]*100)}%")
                axs[i][j].set_title(f"Truth: {amount_real[i*5+j]}% | " + \
                                    f"Prediction: {int(amount_pred[i*5+j])}%")
        plt.tight_layout()
